...
const isAvailableUpdateCritical_nonhook = (availableUpdate: ExpoUpdatesManifest) => {
  ...
  const criticalIndexUpdate = availableUpdate.extra?.expoClient?.extra?.criticalIndex ?? 0;
